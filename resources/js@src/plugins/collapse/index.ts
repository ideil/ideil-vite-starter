const collapse = (toggleEl: HTMLElement) => {
    ...
    const panelEl = document.querySelector(`[data-collapse-panel]${ target }`) as HTMLElement;

    if (!panelEl) {
        return;
    }

    const contentEl = panelEl.querySelector('[data-collapse-content]') as HTMLElement;
    let isOpen = false;
