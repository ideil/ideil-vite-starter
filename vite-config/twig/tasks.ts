...
export function configureTwig({ cache, extensions }: PluginOptions = {}) {
    ...
    if (extensions?.filters) {
        Object.entries(extensions.filters).forEach(([key, fn]) =>
            Twig.extendFilter(key, fn),
        );
    }

    if (extensions?.functions) {
        Object.entries(extensions.functions).forEach(([key, fn]) =>
            Twig.extendFunction(key, fn),
        );
    }
}
...
export async function parseHTML(
    ...
    { fileFilter, fragmentFilter, globals, settings }: PluginOptions = {},
) {
...
export function retrieveTemplatePlaceholders(
    ...
    const matches = html.matchAll(
        /<script\b[^>]*data-template="(?<template>[^>]+)"[^>]*>(?<data>[\s\S]+?)<\/script>/gim,
    );
    const fragments: TwigFragment[] = [];
    ...
    try {
        [...matches].forEach(({ [0]: placeholder, groups = {} }) => {
            ...
            if (
                typeof fragmentFilter !== "function" ||
                fragmentFilter(placeholder, groups.template, data)
            ) {
                fragments.push({
                    data,
                    placeholder,
...
export async function renderTwigTemplate(
    filepath: string,
    context: object,
): Promise<string> {
